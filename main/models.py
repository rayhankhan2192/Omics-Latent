...
# # [REPLACE this function in models.py]
# def create_base_classifier(latent_dim, num_classes):
#     """
#     Creates a simple MLP classifier for a single view's latent features.
#     This is the "Level 1" model in the stacking architecture.
    
#     [V2] This version is simpler and has stronger regularization
#          to combat the extreme overfitting.
#     """
#     input_layer = tf.keras.layers.Input(shape=(latent_dim,))
    
#     # Single hidden layer, stronger L2 penalty
#     x = tf.keras.layers.Dense(
#         32, 
#         activation='relu', 
#         kernel_regularizer=tf.keras.regularizers.l2(1e-3) # Increased L2
#     )(input_layer)
    
#     x = tf.keras.layers.Dropout(0.5)(x) # Keep high dropout
    
#     output_layer = tf.keras.layers.Dense(num_classes, activation='softmax')(x)
    
#     model = tf.keras.models.Model(inputs=input_layer, outputs=output_layer, name="base_classifier_v2")
...
#     """
#     Creates the meta-classifier (Level 2) that takes the 
#     concatenated predictions from all base models as input.
#     Input shape will be (num_views * num_classes).
    
#     [V2] Simpler hidden layer to prevent overfitting.
#     """
#     input_dim_meta = num_views * num_classes
#     input_layer = tf.keras.layers.Input(shape=(input_dim_meta,), name="meta_input")
    
#     # Reduced hidden layer from 32 to 16
#     x = tf.keras.layers.Dense(
#         16, 
#         activation='relu', 
#         kernel_regularizer=tf.keras.regularizers.l2(1e-3) # Increased L2
...
#     model = tf.keras.models.Model(inputs=input_layer, outputs=output_layer, name="meta_classifier_v2")
...
# # MULTI MODEL STACKING CLASSIFIER
# # [ADD THIS to models.py]
# def create_simple_mlp_classifier(fused_input_dim, num_classes):
#     """
#     A simple MLP classifier that takes the single, fused feature vector as input.
#     This serves as a non-attention-based "Level 1" model for stacking.
#     """
...
#     logger.info("Created Simple MLP Classifier (Level 1)")
#     return model
...
#     Input shape will be (num_base_models * num_classes).
#     e.g., 3 models * 5 classes = 15 features.
#     """
#     input_dim_meta = num_base_models * num_classes
#     input_layer = tf.keras.layers.Input(shape=(input_dim_meta,), name="meta_input")
    
#     # A simple, regularized model to learn the weights
...
#     # [ADD THIS to models.py]
# def create_multi_input_classifier(latent_dim_per_view, num_views, num_classes):
#     """
#     Creates a "Late Fusion" model that takes multiple inputs.
    
#     Each view's latent features are fed into a separate input,
#     processed by a small "tower," and then concatenated for
#     final classification.
#     """
#     logger.info("Created Multi-Input 'Late Fusion' Classifier")
    
#     input_layers = []
#     processed_towers = []
    
#     # Create an input and a small processing tower for each view
#     for i in range(num_views):
#         input_layer = tf.keras.layers.Input(shape=(latent_dim_per_view,), name=f"view_{i+1}_input")
        
#         # This is the "tower"
#         x = tf.keras.layers.Dense(64, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(1e-3))(input_layer)
#         x = tf.keras.layers.Dropout(0.4)(x)
        
#         input_layers.append(input_layer)
#         processed_towers.append(x)
        
#     # Concatenate the outputs of all the towers
#     fused = tf.keras.layers.Concatenate()(processed_towers)
    
#     # Final classifier head
#     x = tf.keras.layers.Dense(128, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(1e-3))(fused)
#     x = tf.keras.layers.Dropout(0.5)(x)
...
def create_end_to_end_model(input_dims, latent_dim_per_view, num_classes):
    ...
    # --- [CHANGES] BALANCED REGULARIZATION IN TOWERS ---
    ...
    # --- [CHANGES] BALANCED REGULARIZATION IN CLASSIFIER HEAD ---
...
# def create_end_to_end_model(input_dims, latent_dim_per_view, num_classes):
#     """
#     This is your 'Latent + Original' features model.
#     It's a "Late Fusion" / Multi-Input model that learns its OWN latent
#     features from the ORIGINAL data and classifies them in one step.
    
#     Args:
#         input_dims (list): A list of input dimensions for each view.
#                            e.g., [1000, 1000, 503]
#         latent_dim_per_view (int): The target latent dim for each tower.
#     """
#     logger.info("Creating End-to-End Multi-Input Classifier")
...
#         # This is the "tower" or "encoder" for this view
#         x = tf.keras.layers.Dense(256, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(1e-3))(input_layer)
...
#     # Concatenate the outputs (learned latent features) of all towers
#     fused = tf.keras.layers.Concatenate()(processed_towers)
    
#     # Final classifier head
#     x = tf.keras.layers.Dense(256, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(1e-3))(fused)
#     x = tf.keras.layers.Dropout(0.7)(x)
#     x = tf.keras.layers.Dense(128, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(1e-3))(x)
#     x = tf.keras.layers.Dropout(0.7)(x)
#     output_layer = tf.keras.layers.Dense(num_classes, activation='softmax', name="final_output")(x)
